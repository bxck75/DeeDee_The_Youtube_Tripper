...
if [ $# -ne 4 ]; then
    ...
    echo "./3_frames2movie.sh ffmpeg frames/ MrOizo.mp4 jpg"
    exit 1
fi
...
if [ "avconv" == "$1" ]; then
    ...
    "${FFMPEG}" -loglevel panic -framerate ${FPS} -i "${INFILES}" -c:v ${CODEC} -vf "fps=${FPS},format=yuv420p" -tune fastdecode -tune zerolatency -profile:v baseline "${TMPVIDEO}" -y
